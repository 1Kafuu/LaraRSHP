        ...
        <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
          <a class="flex items-center text-gray-700 dark:text-gray-400" href="#"
            ...
            <span class="mr-3 h-11 w-11 overflow-hidden rounded-full">
              <img src="{{ asset('image/user/owner.jpg') }}" alt="User" />
            </span>

            <span class="text-theme-sm mr-1 block font-medium"> {{ session('user.name') }} </span>
...
@push('scripts')
  <script>
    function initializeSearch() {
      ...
      // Remove existing event listeners first (clean slate)
      ...
      function isUserTypingInInput(element) {
        ...
        if (inputTypes.includes(tagName)) {
          // For input elements, exclude certain types if needed
          if (tagName === 'input') {
      ...
      function updateShortcutButton() {
        ...
        if (isMac) {
          newSearchButton.innerHTML = '<span>⌘</span><span>K</span>';
        } else {
          newSearchButton.innerHTML = '<span>Ctrl</span><span>K</span>';
        }
      }
    ...
    // Also re-initialize when Alpine.js finishes loading (if using Alpine)
    document.addEventListener('alpine:init', initializeSearch);
  </script>
@endpush