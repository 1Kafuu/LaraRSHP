...
<el-dialog>
    <dialog id="create-pet" aria-labelledby="dialog-title"
        ...
        <div tabindex="0" class="flex min-h-full items-center justify-center p-4 sm:p-0">
            <el-dialog-panel
                ...
                <form action="{{ route('createPet') }}" method="POST" class="space-y-6">
                    @csrf

                    <div class="bg-white px-6 pt-5 pb-4">
                        <div class="text-left">
                            <h3 id="dialog-title" class="text-lg font-semibold text-gray-900">Add New Pet</h3>
                            <p class="mt-1 text-sm text-gray-500">Fill in the details below to register the new pet.</p>
                        </div>

                        <div class="mt-5 space-y-4">
                            <!-- Username -->
                            <div>
                                <label for="nama" class="block text-sm font-medium text-gray-700">Nama Pet</label>
                            ...
                            <div>
                                <label for="warna_tanda" class="block text-sm font-medium text-gray-700">Warna
                                    Tanda</label>
                                <input type="text" name="warna_tanda" id="warna_tanda" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-600 focus:ring-indigo-600 sm:text-sm px-3 py-2 border"
                                    placeholder="Enter the pet color mark">
                            </div>
                            ...
                            <div>
                                <label for="nama_ras" class="block text-sm font-medium text-gray-700">Ras Hewan</label>
                                <select name="idras_hewan" id="nama_ras" required
                                    class="jenis-kelamin-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-600 focus:ring-indigo-600 sm:text-sm px-3 py-2 border text-gray-400"
                                    onchange="this.classList.toggle('text-gray-400', this.value === ''); this.classList.toggle('text-gray-900', this.value !== '');">
                                    <option value="" disabled selected>Select pet race...</option>
                                    @foreach ($ras as $row)
                                        <option value="{{ $row->idras_hewan }}">{{ $row->nama_ras }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="bg-gray-50 px-6 py-3 flex flex-row-reverse gap-3">
                        ...
                        <button type="button" command="close" commandfor="create-pet"
...
<div id="updateModal" style="display: none;"
    ...
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 transform transition-all duration-300 scale-95 opacity-0 hidden"
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Update Pet</h3>
            <p class="mt-1 text-sm text-gray-500">Perbarui informasi hewan peliharaan ini.</p>
        </div>
        ...
        <form id="updateForm" method="POST">
            ...
            <input type="hidden" name="idpemilik" id="edit_idpemilik">

            <div class="px-6 py-4 space-y-4">
                <!-- Username -->
                <div>
                    <label for="nama_update" class="block text-sm font-medium text-gray-700 mb-1">Nama Pet</label>
                    <input type="text" name="nama" id="nama_update" required
               ...
               <div>
                    <label for="tanggal_lahir_update" class="block text-sm font-medium text-gray-700">Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" id="tanggal_lahir_update" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-600 focus:ring-indigo-600 sm:text-sm px-3 py-2 border"
                        placeholder="Input the date of birth">
                </div>

                <div class="space-y-1">
                    <label for="warna_tanda" class="block text-sm font-medium text-gray-700">
                        Warna Tanda
                    </label>
                    <input type="text" name="warna_tanda" id="warna_tanda_update" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-600 focus:ring-indigo-600 sm:text-sm px-3 py-2 border"
                        placeholder="Enter the pet color mark">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="jenis_kelamin" id="jenis_kelamin_update" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:border-indigo-600 focus:ring-indigo-600 sm:text-sm">
                        <option value="" disabled selected>Select pet gender...</option>
                        <option value="F">Female</option>
                        <option value="M">Male</option>
                    </select>
                ...
                <div class="mb-4">
                    <label for="pemilik" class="block text-sm font-medium text-gray-700">
                        Pemilik
                    </label>
                    <select name="idpemilik" id="pemilik_update" required
                ...
                <div>
                    ...
                    <select name="idras_hewan" id="nama_ras_update" required
