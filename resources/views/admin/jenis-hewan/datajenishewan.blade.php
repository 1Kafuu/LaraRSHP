...
@push('page')
'{{ $pageName ?? 'datajenishewan' }}'
@endpush

@section('path', 'Data Jenis Hewan')
@section('title', 'Data Jenis Hewan')

@section('content')
    <div class="container-fluid py-4">
        <h1 class="mb-4 text-xl font-bold text-gray-800">Welcome to Data Jenis Hewan</h1>

        <!-- Card Container -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <!-- Header -->
            <div class="flex justify-between items-center p-5 border-b border-gray-100">
                <h5 class="text-lg font-semibold text-gray-800 mb-0">Jenis Hewan List</h5>
                <button 
                    command="show-modal" commandfor="create-jenis"
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-1">
                                        ...
                                        <button
                                            type="button"
                                            onclick="openEditModal({{ $row->idjenis_hewan }})"
                                        ...
                                        <!-- Delete Button -->
                                        <form action="{{ route('deleteJenisHewan', ['id' => $row->idjenis_hewan]) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center text-gray-500 text-base">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="font-medium">Tidak ada data jenis hewan.</p>
                                        <p class="text-sm text-gray-400 mt-1">Klik "Add Jenis Hewan" untuk menambahkan data baru.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
...
@push('scripts')
    <script>
        function openEditModal(jenisId) {
            ...
            // Fetch user data
            fetch(`/getJenisHewanbyId/${jenisId}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    ...
                    document.getElementById('edit_idjenis').value = data.idjenis_hewan;
                    document.getElementById('nama_update').value = data.nama_jenis_hewan;
                    

                    // Update form action
                    const form = document.getElementById('updateForm');
                    form.action = `/updateJenisHewan/${data.idjenis_hewan}`;
                })
                .catch(error => {
