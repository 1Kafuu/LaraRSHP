    ...
    <div class="container-fluid py-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="flex justify-between items-center p-5 border-b border-gray-100">
                ...
                <button command="show-modal" commandfor="create-pemilik"
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-1">
                                        ...
                                        <button
                                            type="button"
                                            onclick="openEditModal({{ $row->idpemilik }})"
                                        ...
                                        <!-- Delete Button -->
                                        <form action="{{ route('deletePemilik', ['id' => $row->idpemilik]) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
...
@push('scripts')
    <script>
        function openEditModal(pemilikId) {
            ...
            // Fetch user data
            fetch(`/getPemilikbyId/${pemilikId}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    ...
                    document.getElementById('nama_update').value = data.user.nama;
                    document.getElementById('email_update').value = data.user.email;
                    document.getElementById('no_wa_update').value = data.no_wa;
                    document.getElementById('alamat_update').value = data.alamat;
                    document.getElementById('edit_iduser').value = data.iduser;

                    // Update form action
                    const form = document.getElementById('updateForm');
                    form.action = `/updatePemilik/${data.idpemilik}`;
