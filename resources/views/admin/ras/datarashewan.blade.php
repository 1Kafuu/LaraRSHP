                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse ($groupRas as $index => $row)
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-colors duration-200 align-top">
                                ...
                                <td class="px-6 py-4 text-gray-500 font-medium" rowspan="{{ $rasCount }}">
                                    {{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}
                                </td>

                                <!-- Jenis Hewan (hanya di baris pertama) -->
                                <td class="px-6 py-4 font-medium text-gray-800" rowspan="{{ $rasCount }}">
                                ...
                                @foreach ($row['ras_hewan'] as $rasIndex => $ras)
                                    @if($rasIndex > 0)
                                        </tr><tr class="hover:bg-indigo-50/30">
                                    @endif
                                    <td class="px-6 py-4">
                                        <span class="inline-block px-3 py-1 text-xs font-medium text-indigo-700 bg-indigo-100 rounded-full">
                                            {{ $ras['nama_ras'] }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center gap-1">
                                            ...
                                            <button
                                                type="button"
                                                onclick="editRasHewan({{ $ras['idras_hewan'] }})"
                                            ...
                                            <button
                                                type="button"
                                                onclick="deleteRasHewan({{ $ras['idras_hewan'] }})"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-gray-500 text-base">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm text-gray-400 mt-1">Klik "Add Ras Hewan" untuk menambahkan data baru.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
