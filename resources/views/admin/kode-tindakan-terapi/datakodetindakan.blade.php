...
@section('content')
    <div class="container-fluid py-4">
        <h1 class="mb-4 text-xl font-bold text-gray-800">Welcome to Data Tindakan</h1>
                ...
                <table class="w-full table-auto">
                    ...
                    <thead class="bg-gray-50 text-indigo-700 font-semibold text-xs uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left">Kode Tindakan</th>
                            <th class="px-6 py-3 text-left">Deskripsi Tindakan</th>
                            <th class="px-6 py-3 text-left">Kategori</th>
                            <th class="px-6 py-3 text-left">Kategori Klinis</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse ($result as $row)
                            <tr class="hover:bg-indigo-50/30 transition-colors duration-200">
                                ...
                                <!-- Kode Tindakan -->
                                <td class="px-6 py-4 font-mono text-indigo-700 font-semibold">
                                    {{ $row->kode ?? '-' }}
                                </td>

                                <!-- Deskripsi Tindakan -->
                                <td class="px-6 py-4 text-gray-800 max-w-md">
                                    <div class="truncate" title="{{ $row->deskripsi_tindakan_terapi }}">
                                        {{ \Illuminate\Support\Str::limit($row->deskripsi_tindakan_terapi, 60) }}
                                    </div>
                                </td>
...
@push('scripts')
    <script>
        function openEditModal(KodeTindakanId) {
            ...
            // Fetch user data
            fetch(`/getKodeTindakanbyId/${KodeTindakanId}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    ...
                    form.action = `/updateKode/${data.idkode_tindakan_terapi}`;
                })
                .catch(error => {
